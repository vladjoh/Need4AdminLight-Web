(function () {
    var storageKey = 'n4al-priv-cols';
    var defs = [
        { key: 'risk', label: 'Exposure' },
        { key: 'upn', label: 'UPN' },
        { key: 'ea', label: 'Entra active roles' },
        { key: 'aa', label: 'Azure active roles' },
        { key: 'mfa', label: 'MFA' },
        { key: 'auth', label: 'Auth methods' },
        { key: 'li', label: 'Last interactive sign-in' },
        { key: 'ln', label: 'Last non-interactive sign-in' }
    ];
    ...
    function saveHidden(map) {
        try { sessionStorage.setItem(storageKey, JSON.stringify(map)); } catch (e) { }
    }
    ...
    function init() {
        ...
        defs.forEach(function (d) {
            ...
            chk.addEventListener('change', function () {
                if (chk.checked) delete hiddenMap[d.key]; else hiddenMap[d.key] = true;
